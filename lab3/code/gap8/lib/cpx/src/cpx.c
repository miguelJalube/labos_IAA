...
void cpxReceivePacketBlocking(CPXFunction_t function, CPXPacket_t * packet) {
  printf("Read blocking for function: %d\n", function);
  xQueueReceive(queues[function], packet, (TickType_t)portMAX_DELAY);
}

int cpxReceivePacket(CPXFunction_t function, CPXPacket_t * packet, TickType_t timeout) {
  return xQueueReceive(queues[function], packet, timeout);
}

void cpxSendPacketBlocking(const CPXPacket_t * packet) {

  if( xSemaphoreTake( tx_sem, ( TickType_t )portMAX_DELAY) == pdTRUE )
  {
